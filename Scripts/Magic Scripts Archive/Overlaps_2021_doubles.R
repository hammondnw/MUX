...
ggplot(data = filter(mux_only)) +
  geom_point(aes(x = DateTime, y = `200`, color = as.character(Valve)))
...
Valve_depth <- data.frame(
  Valve = c (1:12), 
  Depth_m= c("0.1","1.6","3.8","5","6.2", "8", "9", "NA", "acid_r", "air","NA", "water"),
...
mux_only_air = mux_only %>% filter(Depth_m=="air")
...
mux_corrected_long = mux_corrected_long %>% select(DateTime, Depth_m, wavelength, absorbance) %>% 
  arrange(DateTime) %>% filter(Depth_m == "9") %>%
  mutate(Data = "corrected") #%>% rename(absorbance_cor = absorbance)

mux_air_long = mux_air_long %>%  select(DateTime, Depth_m, wavelength, absorbance) %>% 
  arrange(DateTime) %>% mutate(Data = "air") 

plot_df = mux_only_long %>% select(DateTime, Depth_m, wavelength, absorbance) %>%
  filter(Depth_m == "9") %>%  #rename(absorbance_raw = absorbance 
  arrange(DateTime) %>% mutate(Data = "raw")
plot_df = union(plot_df,mux_corrected_long)
plot_df = union(plot_df,mux_air_long)
#plot_df = full_join(plot_df,mux_corrected_long, by = c("DateTime","Depth_m","wavelength", "type","absorbance"))
...
ggplot() +
  ...
  ggtitle("Absorbance at Every 100nm; Depth_m = 9m; Corrected = Raw - Air")
dev.off()

# Plot wavelength vs. absorption for a single time point
ggplot() +
  geom_point(data = filter(mux_only_long, Depth_m == "9" & DateTime > as.POSIXct("2021-06-11 11:00")), 
             aes(x=wavelength,y=absorbance), colour='blue') +
  geom_point(data = filter(mux_corrected_long, Depth_m == "9" & DateTime > as.POSIXct("2021-06-11 11:00")), 
...
dataWQ = dataWQ %>% filter(Reservoir == "FCR" & Site == 50) %>% 
  ...
  filter(DateTime < "2021-06-21 12:00:00") %>% 
...
cleaning = tibble(clean_time =ymd_hms(c("2021-05-26 12:25:00", "2021-05-28 09:30:00", 
                                        "2021-05-31 11:46:00", "2021-06-02 12:22:00", 
                                        "2021-06-04 11:00:00", "2021-06-11 06:00:00", 
                                        "2021-06-07 10:00:00", "2021-06-10 16:55:00", 
                                        "2021-06-15 17:25:00","2021-06-18 10:30:00",
...
for(i in 2:nrow(WQtimes)){ #loop through all sample times and add the closest values to the final dataframe
  ...
  mutate(WQ_DateTime = WQtimes$DateTime[i]) %>% 
    filter(abs(time_diff) < 14400) # 3 hours 
...
MUX_FP_Overlaps_2021 = MUX_FP_Overlaps_2021 %>% select(-c(Depth_m,time_diff,WQ_DateTime))
...
#Write to csv
write.csv(MUX_FP_Overlaps_2021, file = "MUX_FP_Overlaps_2021_doubles.csv")
write.csv(WQ_matches, file = "MUX_WQ_2021_doubles.csv")
