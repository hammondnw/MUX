...
#*        This script can be run for either deployment, just change the 
...
#*           repeated for each variable + depth layer combination (e.g., TFe epi, TFe hypo, SMn epi,...)
...
#load functions
source('./Scripts/Current Magic Scripts/PLSR_MUX_functions.R')
...
FPcaldata_name<-"MUX_FP_Overlaps.csv" # Name of FP 'overlaps' file (for calibration)
...
#2020
Begin_time<- as.POSIXct(c("2020-10-15 12:00"), tz = "Etc/GMT+4")
End_time<- as.POSIXct(c("2020-11-09 15:00:00"), tz = "Etc/GMT+4") 
# 2021
#Begin_time<- as.POSIXct(c("2021-05-26 00:00:00"), tz = "Etc/GMT+4") 
#End_time<- as.POSIXct(c("2021-06-21 14:00:00"), tz = "Etc/GMT+4") 
...
#png("RMSEP20_SMn_epi_5comp_1out_050622.png",width = 9, height = 4, units = 'in', res = 300)
...
#png("Biplot20_SMn_epi_5comp_1out_050622.png",width = 9, height = 4, units = 'in', res = 300)
...
#png("Loading20_SMn_epi_5comp_1out_050622.png",width = 9, height = 5, units = 'in', res = 300)
...
dataWQ$DateTime <- as.POSIXct(dataWQ$DateTime, format="%m/%d/%y %H:%M:%S", tz = "America/New_York")
...
# Plot residuals
#png("Resid20_SMn_epi_5comp_1out_050622.png",width = 9, height = 4, units = 'in', res = 300)
#par(mfrow=c(1,2))
...
# Plot all depths 
#png("Pred20_SMn_epi_5comp_1out_050622.png",width = 9, height = 4, units = 'in', res = 300) 
SMn_plot <- ggplot() +
  geom_path(data=TS_conc, aes(x=DateTime,y=SMn_mgL, color= as.character(Depth_m)), size=0.5) +
  geom_ribbon(data=TS_conc, aes(ymin=uncerSMn_min, ymax=uncerSMn_max, x=DateTime, fill = as.character(Depth_m)), alpha = 0.2)+
  geom_point(data=dataWQ, aes(x=DateTime, y=SMn_mgL, colour= as.character(Depth_m))) +
  #ylim(0,7)+
  labs(x="Date", y = "SMn (mg/L)", title = "PLSR Preds w/ bootstrap PI") +
...
write.csv(TS_conc_all,"./MagicData/MUX/Figures Files/MUX20_predictions_boot_111522.csv")

#write csv of dataWQ
write.csv(WQ_all,"./MagicData/MUX/Figures Files/MUX20_dataWQ_111522.csv")
