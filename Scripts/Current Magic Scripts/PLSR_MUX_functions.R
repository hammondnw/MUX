...
data_prep_20 = function(WQ_name,FPcaldata_name,TimeSeriesFP_name,Depths,Begin_time,End_time,WQparam){
  ...
  colnames(dataCalFP)<-c("DateTime","status",seq(200,732.5,2.5),"Valve", "Measurement Time") #Add column names
  ...
  #Create vector of times from the FP file
  timesCalFP<-dataCalFP %>% select(DateTime)
  
  dataCalFP <- dataCalFP %>% select(!c(DateTime,status)) #Remove DateTime, and status columns
  ...
  #This replaces the Date from the original dataWQ with the exact values
  ...
  #Subset to desired date range
  TS_FP$DateTime = ymd_hms(TS_FP$DateTime, tz = "Etc/GMT+4")
